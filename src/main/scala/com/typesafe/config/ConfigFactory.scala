...
object ConfigFactory {
  ...
  def load(resourceBasename: String): Config = {
    load(resourceBasename, ConfigParseOptions.defaults(), ConfigResolveOptions.defaults())
  }
  ...
  def load(loader: ClassLoader, resourceBasename: String): Config = {
    load(resourceBasename, ConfigParseOptions.defaults().setClassLoader(loader), ConfigResolveOptions.defaults())
  }
  ...
  def load(resourceBasename: String, parseOptions: ConfigParseOptions, resolveOptions: ConfigResolveOptions): Config = {
    ...
    val appConfig = ConfigFactory.parseResourcesAnySyntax(resourceBasename, withLoader)
  ...
  private def checkedContextClassLoader(methodName: String): ClassLoader = {
    val loader = Thread.currentThread().getContextClassLoader
    if (loader == null) throw new ConfigException.BugOrBroken("Context class loader is not set for the current thread; " + 
      "if Thread.currentThread().getContextClassLoader() returns null, you must pass a ClassLoader " + 
      "explicitly to ConfigFactory." + 
      methodName) else loader
  }

  private def ensureClassLoader(options: ConfigParseOptions, methodName: String): ConfigParseOptions = {
    if (options.getClassLoader == null) options.setClassLoader(checkedContextClassLoader(methodName)) else options
  }
  ...
  def load(loader: ClassLoader, config: Config, resolveOptions: ConfigResolveOptions): Config = {
    defaultOverrides(loader).withFallback(config).withFallback(defaultReference(loader))
      .resolve(resolveOptions)
  }

  private def parseApplicationConfig(parseOptions: ConfigParseOptions): Config = {
    val loader = parseOptions.getClassLoader
    if (loader == null) throw new ConfigException.BugOrBroken("ClassLoader should have been set here; bug in ConfigFactory. " + 
    ...
    if (specified == 0) {
      ...
      throw new ConfigException.Generic("You set more than one of config.file='" + file + "', config.url='" + 
        url + 
        "', config.resource='" + 
        resource + 
      ...
      if (resource != null) {
        ...
        parseResources(loader, resource, overrideOptions)
      } else if (file != null) {
        parseFile(new File(file), overrideOptions)
      } else {
        parseURL(new URL(url), overrideOptions)
      }
    }
  }
  ...
  def load(loader: ClassLoader): Config = {
    val withLoader = ConfigParseOptions.defaults().setClassLoader(loader)
   ...
   */
  def defaultReference(loader: ClassLoader): Config = ConfigImpl.defaultReference(loader)
  ...
  def defaultApplication(options: ConfigParseOptions): Config = {
    parseApplicationConfig(ensureClassLoader(options, "defaultApplication"))
  }
  ...
  def parseProperties(properties: Properties, options: ConfigParseOptions): Config = {
    Parseable.newProperties(properties, options).parse()
      .toConfig()
  }
  ...
  def parseURL(url: URL): Config = {
    parseURL(url, ConfigParseOptions.defaults())
  }
  ...
  def parseFileAnySyntax(fileBasename: File, options: ConfigParseOptions): Config = {
    ConfigImpl.parseFileAnySyntax(fileBasename, options)
      .toConfig()
  }
  ...
  def parseResources(klass: Class[_], resource: String, options: ConfigParseOptions): Config = {
    Parseable.newResources(klass, resource, options).parse()
      .toConfig()
  }
  ...
  def parseResourcesAnySyntax(klass: Class[_], resourceBasename: String, options: ConfigParseOptions): Config = {
    ConfigImpl.parseResourcesAnySyntax(klass, resourceBasename, options)
      .toConfig()
  }
  ...
  def parseResourcesAnySyntax(klass: Class[_], resourceBasename: String): Config = {
    parseResourcesAnySyntax(klass, resourceBasename, ConfigParseOptions.defaults())
  }
  ...
  def parseResourcesAnySyntax(loader: ClassLoader, resourceBasename: String, options: ConfigParseOptions): Config = {
    ConfigImpl.parseResourcesAnySyntax(resourceBasename, options.setClassLoader(loader))
      .toConfig()
  }
  ...
  def parseResourcesAnySyntax(loader: ClassLoader, resourceBasename: String): Config = {
    parseResourcesAnySyntax(loader, resourceBasename, ConfigParseOptions.defaults())
  }
  ...
  def parseResources(resource: String, options: ConfigParseOptions): Config = {
    val withLoader = ensureClassLoader(options, "parseResources")
    Parseable.newResources(resource, withLoader).parse()
      .toConfig()
  }
  ...
  def parseResourcesAnySyntax(resourceBasename: String, options: ConfigParseOptions): Config = {
    ConfigImpl.parseResourcesAnySyntax(resourceBasename, options)
      .toConfig()
  }
   ...
   */
  def parseMap(values: Map[String, _ <: Any], originDescription: String): Config = {
   ...
   * @return the map converted to a {@code Config}
   */
  def parseMap(values: Map[String, _ <: Any]): Config = parseMap(values, null)
}
